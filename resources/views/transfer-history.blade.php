@extends('layouts.app', ['title' => 'Transfer History - Retrixnet'])
        ...
        <div class="container">
            <div class="col-lg-12 mx-auto">
                <div class="row g-3">
                    ...
                    <div class="col-lg-9">
                        <h4 class="fw-bold"><img src="{{ asset('img/transfer-history.png') }}" width="50"
                                height="50" alt=""> Transfer History</h4>
                        <div class="card border-0">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-lg-7">
                                    ...
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="table-responsive">
                                                <small class="text-muted">All Transfers</small>
                                                <table class="table align-middle">
                                                    ...
                                                    <tbody id="allTxBody">
                                                        <tr>
                                                            <td colspan="6" class="text-muted">No transfers yet.</td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
    ...
    <script>
        async function hydrateAllTx() {
            ...
            (data.data || []).forEach(t => {
                ...
                tr.innerHTML =
                    `<td>${t.type}</td><td>${t.asset_symbol ?? t.asset_id}</td><td>${t.amount}</td><td>${t.status}</td><td>${t.network_name ?? ''}</td><td>${t.tx_hash ?? ''}</td>`;
                tbody.appendChild(tr);
            });
            if (!tbody.children.length) {
                const tr = document.createElement('tr');
                tr.innerHTML = '<td colspan="6" class="text-muted">No transfers yet.</td>';
                tbody.appendChild(tr);
            }
        }
        document.addEventListener('DOMContentLoaded', hydrateAllTx);
    </script>
@endpush
